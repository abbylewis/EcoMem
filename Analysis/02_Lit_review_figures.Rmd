...
country_levels <- c(
  "Finland", "Canada", "Poland", "United States",
...
lake <- read_delim("../01a_Raw_data/WoS_lake_08Dec25.txt",
  delim = "\t",
...
season_plot_pres <- all_seas %>%
  select(-seasonality) %>%
  pivot_longer(c(season)) %>%
  mutate(name = paste0('"', name, '"')) %>%
  left_join(lake, by = c("PY" = "Year")) %>%
  filter(PY >= 1980, PY <= 2024) %>%
  ggplot(aes(x = PY, y = value / Count * 100)) +
  geom_point(alpha = 0.3) +
  ylab('% of all lake papers') +
  geom_smooth(method = "gam", color = "black", linewidth = 0.5) +
  egg::theme_presentation() +
  theme(axis.title.x = element_blank()) +
  facet_wrap(~name, scales = "free_y", ncol = 1)

png("../03a_Figures/Total_prevalence_pres.png",
  res = 300, width = 7,
  height =6, units = "in"
)
season_plot_pres
...
sums <- read_csv("../05_Lit_review/season_summaries/sums.csv") %>%
  mutate(class = factor(class, levels = c("Temperate", "Other")))
...
label_data_pct <- sums %>%
  ...
  filter(PY == 2024) %>%
  ungroup() %>%
  filter(AllSeasons %in% c(
    "spring", "summer", "autumn", "winter", "wet",
    "rainy", "dry", "warm", "growing", "breeding"
  )) %>%
  mutate(pct_of_all_by_year = case_when(AllSeasons == "autumn" ~ pct_of_all_by_year + 0.3,
    AllSeasons == "growing" ~ pct_of_all_by_year + 0.05,
    .default = pct_of_all_by_year
  ))
...
timeline <- sums %>%
  ...
  geom_text(aes(x = 2025, label = AllSeasons),
    hjust = 0,
    vjust = 0.2,
    size = 3,
    data = label_data_pct,
...
# Combine and save
png("../03a_Figures/Total_prevalence_comb.png",
  res = 300, width = 5.5,
  height = 3.5, units = "in"
)
ggpubr::ggarrange(
  all_season_plot +
    theme(panel.border = element_rect(linewidth = 0.3)),
  timeline +
    theme(
...
thirdLevel <- season_word_counts_plot %>%
  ...
  rowwise() %>%
  mutate(
    angle = compute_angle(pos / sum_seas),
...
og <- ggplot() +
  ...
  geom_text(
    data = thirdLevel,
    aes(
      label = label, x = 6.2, y = pos, angle = angle,
      hjust = hjust, color = Type
    ), show.legend = F
  ) +
  xlim(c(0, 8)) +
  coord_polar("y") +
...
og
ggsave(plot = og, "../03a_Figures/Sunburst_og.jpg", width = 4, height = 6)
```

Same as above, but faceted by country
